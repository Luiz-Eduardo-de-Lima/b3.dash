...
def get_statements(begin = int, end = int):
    ...
    for stt in statements:
        for stt_tp in statements_type:
            ...
            data.to_csv(f'statements/{stt}_{stt_tp}_from_{begin}_to_{end}.csv')
    ...
    data.to_csv(f'statements/itr_cia_aberta_from_{begin}_to_{end}.csv')      
    return

current_year = int(datetime.now().strftime('%Y'))
get_statements(2011,current_year)